  ...
  server.add(abi, [
    {
      ...
      func: async (args: ethers.utils.Result) => {
        ...
        const decodedName = packet.name.decode(Buffer.from(name.slice(2), 'hex'));
        const result = await prover.queryWithProof(qtype, decodedName);
        const ret = [result.answer].concat(result.proofs)
          .map((entry) => ({rrset: entry.toWire(), sig: entry.signature.data.signature}));
        console.log({ret});
        return [ret];
      }
    }
